...
#endif
{
    ...
    public virtual void Recalibrate(bool instant)
    {
        StartCoroutine(FadeUp(1, instant));
    }
    ...
    protected GameObject mTransitionFirstTime;
    ...
    protected virtual void Awake()
    {
        ...
        // Neck
        mNeck = Controller.transform.Find("Neck");
    ...
    protected virtual void Start()
    {
        ...
        mTransitionFirstTime = Controller.TransitionCanvas().transform.Find("First Time").gameObject;

        if (sFirstTime)
        {
            Controller.TransitionCanvas().SetActive(true);
            mTransitionFirstTime.SetActive(true);
        }

        // Override clip planes
        Controller.Head.GetComponent<Camera>().nearClipPlane = NearClipPlane;
        Controller.Head.GetComponent<Camera>().farClipPlane = FarClipPlane;
    ...
    protected void UpdateNeck()
    {
        ...
        mNeck.transform.eulerAngles = new Vector3(0, rotY + Controller.YawOffset, 0);

        mNeck.transform.localPosition = new Vector3(Controller.XOffset, NeckHeight + Controller.YOffset, Controller.ZOffset);
    }
    ...
    protected virtual void UpdateCalibrating()
    {
        ...
        if (Controller.LeftButton.Held(0.5f) && Controller.RightButton.Held(0.5f) && Controller.IsHeadTracked())
        {
            ...
            StartCoroutine(Recenter(sFirstTime));

            sFirstTime = false;
        }
    }

    protected IEnumerator FadeUp(float fadeTime, bool instant)
    {
        ...
        {
            time += Time.deltaTime;
            group.alpha = Mathf.SmoothStep(0.0f, 1.0f, time / fadeTime);
            yield return null;
        }

        // Goto calibration mode
        if (instant)
        {
            StartCoroutine(Recenter(false));
        }
        else
        {
            SetState(kStateCalibrating);
        }
    }

    protected IEnumerator Recenter(bool restart)
    {
        // Reset oculus position
        Controller.Calibrate();
        yield return null;

        float killTime = Time.time + 5;

        while (Controller.Calibrating() && Time.time < killTime)
        {
            yield return null;
        }
        ...
        {
            Restart(true); // resets state to normal
        }
        else
        {
            if (State() == kStateCalibrating)
